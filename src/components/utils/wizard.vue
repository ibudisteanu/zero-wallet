  ...
  <div class="theme-wizard" ref="wizard">
    <div class="card-header bg-light pt-2 pb-2">
      <ul class="nav justify-content-between nav-wizard">
        <li v-for="(titleIndex, index ) in titlesSorted" class="nav-item" :key="`tab_title_${index}`">
          <span :class="`nav-link fw-semi-bold ${getIconClass(titleIndex)} ${titleIndex < tab ? 'cursor-pointer': ''}`"
                v-tooltip.bottom="`${titles[titleIndex].tooltip}`" @click="titleIndex < tab ? tab = titleIndex : null ">
    ...
    <div class="card-body py-3">
      <div class="tab-content px-3">
        <div v-for="(titleIndex, index) in titlesSorted" :class="`tab-pane ${tab===titleIndex?'active':''}`" :key="`tab_option_${index}`">
    ...
    <div :class="controlsClassName">
      ...
      <ul class="pager wizard list-inline mb-0">
        <li class="previous">
          <loading-button v-if="tab > start" text="Back" :submit="handleBack" icon="fas fa-chevron-left ms-2"
                          classCustom="btn btn-link" :iconLeft="false"/>
        </li>
        <li class="next">
          <loading-button v-if="tab <= end && (!buttons[tab] || !buttons[tab].hide)"
                          :text="`${ buttons[tab] ? buttons[tab].text : 'Next'}`" :submit="handleNext"
                          :icon="`${ buttons[tab] ? buttons[tab].icon : 'fas fa-chevron-right ms-2' }`"/>
        </li>
      </ul>
  ...
  methods: {
    ...
    handleNext() {
      return new Promise((resolve, reject)=>{
        for (let i = 0; i < this.titlesSorted.length; i++)
          if (this.titlesSorted[i] === this.tab)
            return this.setTab(resolve, (i < this.titlesSorted.length - 1) ? this.titlesSorted[i + 1] : this.titlesSorted[i] + 1)
        resolve()
      })
    },
  },
