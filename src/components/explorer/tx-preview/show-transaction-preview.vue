    ...
    <template v-else>
      ...
      <div class="col-8 col-md-1">
        <template v-if="tx.version.eq( PandoraPay.enums.transactions.TransactionVersion.TX_SIMPLE )">
          <span :class="`text-truncate badge badge-soft-${$store.getters.getTxScriptBadgeColor(tx.version, tx.base.txScript)} d-md-flex justify-content-center align-items-center`"
              v-tooltip.bottom="$store.getters.getTxScriptText(tx.version, tx.base.txScript)">
            {{ $store.getters.getTxScriptTextShort(tx.version, tx.base.txScript) }}
          </span>
        </template>
        <template v-else-if="tx.version.eq( PandoraPay.enums.transactions.TransactionVersion.TX_ZETHER )">
          <span v-for="(payload, index) in tx.base.payloads" :key="`tx_type_${index}`" class="col-12">
            <span :class="`text-truncate badge badge-soft-${$store.getters.getTxScriptBadgeColor(tx.version, payload.payloadScript)} d-md-flex justify-content-center align-items-center`"
                  v-tooltip.bottom="$store.getters.getTxScriptText(tx.version, payload.payloadScript)">
              {{ $store.getters.getTxScriptTextShort(tx.version, payload.payloadScript) }}
            </span>
          </span>
        </template>
      </div>
      ...
      <div class="col-8 col-md-2 text-truncate">
        ...
        <template v-else-if="tx.version.eq( PandoraPay.enums.transactions.TransactionVersion.TX_ZETHER )">
          <div v-for="(payload, index) in tx.base.payloads" :key="`tx_payload_${index}`"
            ...
            <template v-if="payload.dataVersion.eq( PandoraPay.enums.transactions.TransactionDataVersion.TX_DATA_PLAIN_TEXT)">
              <span v-tooltip.bottom="`${Buffer.from(payload.dataPublic, 'base64').toString()}`">
                {{ Buffer.from(payload.dataPublic, "base64").toString() }}
              </span>
            </template>
      ...
      <div class="col-8 col-md-2 text-truncate">
        <div v-if="tx.version.eq( PandoraPay.enums.transactions.TransactionVersion.TX_ZETHER )">
          <loading-button v-if="canDecrypt && !decrypted" type="button"
                          tooltip="Decrypt the transaction to see the amount, shared text and recipient"
                          :submit="decryptTx" text="Decrypt" icon="fas fa-unlock" :icon-left="false"
                          class-custom="badge rounded-pill bg-info" component="span"/>
