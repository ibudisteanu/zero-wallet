<template>
  ...
  <layout>
    ...
    <simple-tx :public-key="publicKey" @onSetTab="setTab" account-type="none" :titles-offset="titlesOffset"
               :enable-fee="false" :enable-sender="false" :before-process="beforeProcess" >
          ...
          <div v-for="(sig, i) in signatures" :key="i" class="my-4">
            ...
            <div class="row">
              <div class="col-12">
                ...
                <input class="form-control" type="text" v-model="sig.signature">
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  data(){
    return {
      titlesOffset:  {
        '-2': { icon:"fas fa-dollar-sign", name: "Conditional Payment", tooltip: "Details regarding the Transaction Payload"},
        ...
        if (oldTab === -1 && value === 0 ){
          ...
          for (const sig of this.signatures)
            if ( sig.signature !== null) {
              ...
              let s
              try{
                s = Buffer.from(sig.signature, "base64")
              }catch(e){
  ...
  watch: {
    $route:{
      ...
      handler: function (to, from) {
        ...
        try{
          if (!to.params.txId.length && to.params.txId.length !== 2*PandoraPay.cryptography.HASH_SIZE)
            throw "Invalid Tx Id"

          this.payloadIndex = to.params.payloadIndex
