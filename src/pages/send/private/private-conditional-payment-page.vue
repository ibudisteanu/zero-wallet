        ...
        <zether-tx :public-key="publicKey" @onSetTab="setTab" ref="zetherTx"
            ...
            <template v-slot:tab_0.5>
                ...
                <div class="col-12">
                    ...
                    <input :class="`form-control ${validationDeadline ? 'is-invalid': ''}`" type="number"
                           v-model.number="deadline" max="100000" min="10"/>
                ...
                <div class="col-12 mt-3">
                    ...
                    <i class="fas fa-question ms-1" v-tooltip.bottom="`Minimum number of signatures to process the transaction.`"/>
                ...
                <div class="mt-2">
                    <div class="col-12" v-for="(key, i) in multisigPublicKeys" :key="`multisig-public-key-${i}`">
                        ...
                        <div class="input-toggle-group">
                            ...
                            <i v-if="i >= 1" class="fas fa-times input-toggle" :style="`${validationPublicKeys[i] ? 'right: 35px' : ''}`" @click="()=>handleDeleteMultisigPublicKey(i)"/>
                            <div v-if="validationPublicKeys[i]" class="invalid-feedback d-block">{{ validationPublicKeys[i] }}</div>
                        </div>
                    </div>
                </div>

                <button class="btn btn-falcon-default rounded-pill me-1 mt-3  pointer" type="button" :disabled="multisigPublicKeys.length >= 5"
                        @click="handleAddNewMultisigPublicKey">
                    <i class="fa fa-plus"/>
